...
<div class="min-h-screen bg-[#000000] pt-20 lg:pt-32 pb-12">
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="text-center mb-8">
            ...
            <h1 class="text-3xl font-bold text-white mb-2">{{ app()->getLocale() === 'en' ? 'Payment Successful!' : 'Pembayaran Berhasil!' }}</h1>
            <p class="text-gray-400">{{ app()->getLocale() === 'en' ? 'Thank you for your payment. Your order is being processed.' : 'Terima kasih atas pembayaran Anda. Pesanan sedang diproses.' }}</p>
        </div>
        ...
        <div class="bg-[#0E0E10] rounded-2xl border border-white/5 overflow-hidden mb-6">
            <div class="bg-linear-to-r from-emerald-500/10 to-emerald-600/5 px-6 py-4 border-b border-white/5">
                <h2 class="text-lg font-semibold text-white flex items-center gap-2">
                    ...
                    {{ app()->getLocale() === 'en' ? 'Order Details' : 'Detail Pembelian' }}
                </h2>
            </div>
            ...
            <div class="p-6 space-y-4">
                ...
                <div class="flex justify-between items-start">
                    <span class="text-gray-400 text-sm">{{ app()->getLocale() === 'en' ? 'Transaction ID' : 'ID Transaksi' }}</span>
                    <span class="text-white font-mono font-semibold">{{ $data['trxid'] }}</span>
                </div>

                <!-- Product -->
                <div class="flex justify-between items-start">
                    <span class="text-gray-400 text-sm">{{ app()->getLocale() === 'en' ? 'Product' : 'Produk' }}</span>
                ...
                @if($data['type'] === 'game')
                    <!-- User ID -->
                    <div class="flex justify-between items-start">
                        <span class="text-gray-400 text-sm">{{ app()->getLocale() === 'en' ? 'User ID' : 'User ID' }}</span>
                    ...
                    <!-- Zone ID -->
                    <div class="flex justify-between items-start">
                        <span class="text-gray-400 text-sm">{{ app()->getLocale() === 'en' ? 'Zone ID' : 'Zone ID' }}</span>
                ...
                @else
                    <!-- Phone Number -->
                    <div class="flex justify-between items-start">
                        <span class="text-gray-400 text-sm">{{ app()->getLocale() === 'en' ? 'Phone Number' : 'Nomor Tujuan' }}</span>
                        <span class="text-white font-medium">{{ $data['data_no'] }}</span>
                    </div>
                @endif
                ...
                <!-- Payment Method -->
                <div class="flex justify-between items-start">
                    <span class="text-gray-400 text-sm">{{ app()->getLocale() === 'en' ? 'Payment Method' : 'Metode Pembayaran' }}</span>
                ...
                <!-- Status -->
                <div class="flex justify-between items-start">
                    <span class="text-gray-400 text-sm">{{ app()->getLocale() === 'en' ? 'Status' : 'Status' }}</span>
                    @php
                ...
                <!-- Date -->
                <div class="flex justify-between items-start">
                    <span class="text-gray-400 text-sm">{{ app()->getLocale() === 'en' ? 'Date' : 'Tanggal' }}</span>
        ...
        <div class="flex flex-col sm:flex-row gap-3">
            ...
            <a href="{{ localized_url('/') }}" 
               class="flex-1 flex items-center justify-center gap-2 bg-white/5 hover:bg-white/10 text-white font-semibold px-6 py-3 rounded-xl border border-white/10 transition-colors">
                ...
                <span>{{ app()->getLocale() === 'en' ? 'Back to Homepage' : 'Kembali ke Beranda' }}</span>
            </a>
        </div>
