<!-- News Section -->
    <div class="bg-[#000000] px-3 lg:px-8 py-6 lg:py-8">
        <div class="max-w-7xl mx-auto">
            <h1 class="text-gray-100 font-semibold text-xl lg:text-2xl mb-4 lg:mb-6">{{ app()->getLocale() === 'en' ? 'Latest News' : 'Berita Terbaru' }}</h1>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 lg:gap-4">
                ...
                <div class="col-span-full text-center py-8">
                    <p class="text-gray-400 text-sm lg:text-base">{{ app()->getLocale() === 'en' ? 'No news available' : 'Belum ada berita tersedia' }}</p>
                </div>
                @endforelse
            </div>
            ...
            <div class="mt-4 lg:mt-6 text-center">
                <button class="text-yellow-500 hover:text-yellow-400 font-semibold text-sm lg:text-base underline transition-colors">{{ app()->getLocale() === 'en' ? 'Read More' : 'Baca Lebih Lanjut' }}</button>
            </div>
            @endif
        </div>
    </div>