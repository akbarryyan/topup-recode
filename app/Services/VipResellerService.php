...
class VipResellerService
{
    ...
    /**
     * Check Prepaid Order Status from VIP Reseller
     * 
     * @param string $trxid Transaction ID from VIP Reseller
     * @return array
     */
    public function checkPrepaidOrderStatus(string $trxid): array
    {
        if (!$this->ensureConfigured()) {
            return [
                'success' => false,
        ...
        try {
            ...
            Log::info('VIP Reseller Check Prepaid Status Request', [
                'url' => $this->apiUrl . '/prepaid',
                'trxid' => $trxid,
            ]);
            ...
            Log::info('VIP Reseller Check Prepaid Status Response', [
                'status' => $response->status(),
                'body' => $response->body(),
            ]);
            ...
            Log::error('VIP Reseller Check Prepaid Status Error: ' . $e->getMessage(), [
                'trxid' => $trxid,
                'trace' => $e->getTraceAsString(),
            ]);
